import { ParamDefinition, ValidationResult, ParamType } from '../types/validation';
...
export class ParamValidator {
    ...
    static async validateParam(
        ...
        if (definition.required && (value === null || value === undefined)) {
            return {
                valid: false,
                message: definition.errorMessage || `Parameter '${paramName}' is required`,
                field: paramName,
            };
        }
        ...
        if (!definition.canBeEmpty && value === '') {
            return {
                valid: false,
                message: definition.errorMessage || `Parameter '${paramName}' cannot be empty`,
                field: paramName,
            };
        }

        // Type validation
        const typeValidation = this.validateType(paramName, value, definition.type);
        ...
        if (typeof value === 'string') {
            if (!CommonValidators.isValidLength(value, definition.min, definition.max)) {
                return {
                    valid: false,
                    message:
        ...
        if (typeof value === 'number') {
            if (!CommonValidators.isInRange(value, definition.min, definition.max)) {
                return {
                    valid: false,
                    message:
                        definition.errorMessage ||
        ...
        if (definition.pattern && typeof value === 'string') {
            if (!definition.pattern.test(value)) {
                return {
                    valid: false,
                    message:
                        definition.errorMessage || `Parameter '${paramName}' format is invalid`,
                    field: paramName,
                };
            }
        }
     ...
     */
    private static validateType(paramName: string, value: any, type: ParamType): ValidationResult {
        switch (type) {
            case 'string':
                return {
            ...
            case 'email':
                return {
                    valid: typeof value === 'string' && CommonValidators.isValidEmail(value),
                    message: `${paramName} must be a valid email`,
                };

            case 'password':
                return {
                    valid: typeof value === 'string' && CommonValidators.isValidPassword(value),
                    message: `${paramName} must be a valid password (min 8 chars, uppercase, lowercase, number, special char)`,
                };

            case 'phone':
                return {
                    valid: typeof value === 'string' && CommonValidators.isValidPhone(value),
            ...
            case 'url':
                return {
                    valid: typeof value === 'string' && CommonValidators.isValidUrl(value),
                    message: `${paramName} must be a valid URL`,
                };

            case 'uuid':
                return {
                    valid: typeof value === 'string' && CommonValidators.isValidUuid(value),
    ...
    static async validateParams(
        ...
        for (const definition of definitions) {
            ...
            const result = await this.validateParam(definition.name, value, definition);
            if (!result.valid) {
                results.push(result);
            }
        }
